...
### BUG-05 — Pesan error login berbahasa Inggris
- **Tingkat:** Minor (i18n)
- **Gejala:** Saat kredensial salah, muncul *"These credentials do not match our records."*
- **Akar masalah:** Pesan default Laravel (`auth.failed`), belum di-override ke Indonesia.
- **Perbaikan:** Menambah `lang/en/auth.php` (override terkurung, locale tetap `en`):
  - `failed` → "Email atau password salah."
  - `throttle` → "Terlalu banyak percobaan masuk. Silakan coba lagi dalam :seconds detik."
- **Verifikasi:** `php artisan tinker --execute="echo trans('auth.failed');"` → "Email atau password salah."
